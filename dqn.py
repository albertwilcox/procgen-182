...
class QLearner(object):
    ...
    def __init__(self,
        ...
        # START OF YOUR CODE
        # ----------------------------------------------------------------------
        self.qs_vals = q_func(obs_t_float, self.num_actions, 'q_func')
        q_vals = tf.linalg.diag_part(tf.gather(self.qs_vals, self.act_t_ph, axis=1))
        print(self.qs_vals.shape, q_vals.shape)

        self.target_qs_vals = q_func(obs_tp1_float, self.num_actions, 'target_q_func')
        # self.target_qs_vals = self.qs_vals
        if self.double_q:
            target_actions = tf.math.argmax(self.qs_vals, axis=1)
            target_q_vals = tf.linalg.diag_part(tf.gather(self.target_qs_vals, target_actions, axis=1))
        else:
            target_q_vals = tf.reduce_max(self.target_qs_vals, axis=1)
        print(self.target_qs_vals.shape, target_q_vals.shape)

        q_func_vars = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='q_func')
        target_q_func_vars = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='target_q_func')

        [print('    ' + str(var)) for var in q_func_vars]

        target = self.rew_t_ph + (1 - self.done_mask_ph) * gamma * target_q_vals
        self.total_error = tf.reduce_mean(huber_loss(target - q_vals))
        # ----------------------------------------------------------------------
        # END OF YOUR CODE
        # ----------------------------------------------------------------------
    ...
    def step_env(self):
        ...
        if not self.model_initialized or np.random.random() < ep:
            ...
            outputs = self.session.run(self.qs_vals, feed_dict={self.obs_t_ph: last_obs.reshape([1] + list(last_obs.shape))})
            a = np.argmax(outputs)
    ...
    def update_model(self):
        ...
        variable `self.num_param_updates` useful; it was initialized to 0.
        """
        if (self.t > self.learning_starts and \
            ...
            # print(done_mask)
            # if np.max(rew_batch) != np.min(rew_batch) or True:
            #     print(rew_batch)
            #     print('\n\n\n\n\n')

            loss, _ = self.session.run((self.total_error, self.train_fn),
                                       feed_dict={self.obs_t_ph: obs_batch, self.obs_tp1_ph: next_obs_batch,
                                                  self.act_t_ph: act_batch, self.rew_t_ph: rew_batch,
